  ...
  return (
    <div
      className={classnames(
        isActive ? "bg-primary-dark" : "bg-[#070809]",
        baseClasses
      )}
      onClick={onClick}
      onKeyDown={onClick}
    ...
    >
      <div className="flex flex-1 flex-col px-4 pb-2">
        <div className="flex flex-row items-center justify-between pt-2 pb-2">
          ...
          <div className="flex flex-row items-center text-base text-primary-light">
            <Icon
              name="group-layers"
              className="mx-2 w-4 text-primary-light"
            />
            {numInstances}
          </div>
        </div>
        <div className="flex flex-row py-1">
          <div className="pr-5 text-xl text-primary-light">{modalities}</div>
          <div className="truncate-2-lines break-words text-base text-primary-light">{description}</div>
        </div>
      </div>
      {!!trackedSeries && (
